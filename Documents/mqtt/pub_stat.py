...
def pubTempData(client, freq=10, limit=100):
    try:    
        ...
        for i in range(limit*freq):
            ...
            mem_use = float(psutil.virtual_memory().used)/psutil.virtual_memory().total * 100 #(percentige )#psutil.virtual_memory().used/(1024**2)# mb
            #row = "Time : {:s}, Cpu temp: {:s}%, Cpu use: {}%, Mem total: {:.1f} MB, Mem use: {:.1f} MB".format(ti.strftime("%Y-%m-%d %H:%M:%S.%f"),da, cpu_use, mem_tot, mem_use)
            row = "{:s}, {:s}, {}, {:.1f}, {:.1f}".format(ti.strftime("%Y-%m-%d %H:%M:%S.%f"),da, cpu_use, mem_tot, mem_use)
