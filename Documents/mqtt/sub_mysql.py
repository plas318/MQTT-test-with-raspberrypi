...
def on_message(client, userdata, msg):
    ...
    try:    
        ...
        #print(row_data)
        sub_data ={k:0 for k in keys}
        rec_time = datetime.strptime(row_data[0], "%Y-%m-%d %H:%M:%S.%f")
        sub_data['cpu_temp'] = float(row_data[1])
        sub_data['cpu_use'] = float(row_data[2])
        sub_data['mem_use']= float(row_data[4])
        sub_data['mem_total']= float(row_data[3])
        sample_count += 1
        ...
        if sample_count >= sample_max:
            ...
            avg_data = [float(sum_data[k] / sample_count) for k in keys]
            str_data = '{:.1f}, {:.1f}, {:.1f}, {:.1f}'.format(*avg_data) # cpu temp
            sample_count = 0
            for k in keys:
                sum_data[k] = 0
...
if __name__ == "__main__":
    ...
    client.connect(host =MQ_HOST, port=1883,keepalive=120)
